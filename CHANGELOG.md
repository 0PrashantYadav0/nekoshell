...
- Three more, for the landing page: `docs/screenshots/greet-pokemon.png` and `greet-pokemon-latte.png`, the greeting in iTerm2 with the pokemon provider in mocha and after `nekoshell theme latte`, Wi-Fi and IP painted out like the others; and `doctor.png`, the real output of `nekoshell doctor` recorded through a pty and drawn into an iTerm2-style window at 1600 px rather than captured from the screen.
