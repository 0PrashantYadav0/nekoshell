...
**Images.** No third-party artwork as such anywhere in the repository: no sprite, still or texture copied in. The one exception is a screenshot of the running program, since it shows the program, with whatever art it drew at the time; those are kept small and out of the tarball. `plugins/greet/art/` holds the project's own pixel art only.

**Screenshots.** A new screenshot goes in `docs/screenshots/`, named `greet-<provider>.png`, `terminal-<id>.png` or `<plugin>-<what>.png`, 1600 px wide (`sips -Z 1600 FILE`) and run through `pngquant` (`pngquant --quality 65-90 --speed 1 --strip --force --ext .png FILE`) until it is under 250 KB. `docs/screenshots/` is listed in `.gitattributes` with `export-ignore`, so it stays out of the release tarball. A screenshot never shows a Wi-Fi network name, an IP address, a token or anything else private; hide those fastfetch rows before taking it, or paint them out.

**Vendored files.** A file copied from another project is listed in [THIRD_PARTY.md](THIRD_PARTY.md) with its source commit and license. Generated files (`core/theme/palettes.json`, `plugins/pokemon/data/pokemon.tsv`) are committed with the generator beside them; re-run the generator rather than editing the output.
