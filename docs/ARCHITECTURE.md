...
`core/theme/palettes.json` holds the four Catppuccin flavours (latte, frappe, macchiato, mocha), each mapping 26 role names to a hex string with no leading `#`. It is generated by `core/theme/gen-palettes.py` from catppuccin/palette at a commit pinned in the script, and it is the only place in the repository a colour is written down.
