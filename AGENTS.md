...
Write `terminals/<id>/README.md` with `## What it configures`, `## Panel`, `## Images`, `## Uninstall` and `## Known limits`, add a row and a link to [docs/TERMINALS.md](docs/TERMINALS.md), add `tests/terminals/<id>.bats`, and add the id to the `install` matrix in `.github/workflows/ci.yml`.

### Docs

`docs/README.md` is the index and names every page or the index that holds it; a new page joins one of its four groups. The pages a user reads ship in the release tarball: `docs/INSTALL.md`, `docs/CONFIGURATION.md` (`nekoshell.toml`, the themes, the prompt, `greet.conf`, `local.zsh`), `docs/TERMINALS.md` (what the five terminals can do), `docs/ARCHITECTURE.md`, `docs/DISTRIBUTION.md`, `docs/REMOTE.md`, `docs/plugins/` and `terminals/<id>/README.md`. The contributor-only paths do not: `docs/contributing/`, `docs/ci-checks/`, `docs/ai/`, `docs/superpowers/`, `docs/screenshots/`, `docs/assets/`, `tests/`, `scripts/`, `skills/`, `packaging/`, the `Makefile`, this file, `CLAUDE.md`, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `SECURITY.md` and `llms.txt` are all `export-ignore` in `.gitattributes`. A new development-only path gets an `export-ignore` line of its own, and a link from a page that ships into a path that does not is written as `https://github.com/0PrashantYadav0/nekoshell/blob/main/<path>` rather than as a relative link.

A screenshot goes in `docs/screenshots/` as `greet-<provider>.png`, `terminal-<id>.png` or `<plugin>-<what>.png`, 1600 px wide (`sips -Z 1600 FILE`) and run through `pngquant --quality 65-90 --speed 1 --strip --force --ext .png FILE` until it is under 250 KB; the README shows one, the whole window at full width, and the plugin pages and `docs/TERMINALS.md` show the rest; every page references it with a relative path, because GitHub renders it from the repository and a tarball reader loses only a picture. The mascot in `docs/assets/` is a model and a render made by `docs/assets/gen-neko.py`; edit the script and rerun it rather than editing the outputs.
