...
A screenshot goes in `docs/screenshots/` as `greet-<provider>.png` or `<plugin>-<what>.png`, 1600 px wide (`sips -Z 1600 FILE`) and run through `pngquant --quality 65-90 --speed 1 --strip --force --ext .png FILE` until it is under 250 KB; the README's table and the plugin pages reference it with a relative path, because GitHub renders it from the repository and a tarball reader loses only a picture.
