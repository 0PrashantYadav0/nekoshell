...
link_tree() {
  ...
  while IFS= read -r f; do
    ...
    if [[ "${NEKOSHELL_DRY_RUN:-0}" == "1" ]]; then log_info "would link $rel"; continue; fi
...
unlink_tree() {
  ...
  while IFS= read -r f; do
    ...
    if [[ "${NEKOSHELL_DRY_RUN:-0}" == "1" ]]; then log_info "would unlink $rel"; continue; fi
    rm -f "$target"
  done < <(_link_files "$src")
}
...
copy_once() {
  ...
  while IFS= read -r f; do
    ...
    if [[ "${NEKOSHELL_DRY_RUN:-0}" == "1" ]]; then log_info "would copy $rel"; continue; fi
    mkdir -p "$(dirname "$target")"
    cp "$f" "$target"
  done < <(_link_files "$src")
}
