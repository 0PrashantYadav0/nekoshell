...
cmd_terminal() {
  ...
  case "$action" in
    detect)
      local t
      if t="$(terminal_detect_env)"; then echo "$t"; else echo "unknown"; return 1; fi
      ;;
    list) _terminal_cmd_list ;;
    use)
      local id="${1:-}"
      [[ -n "$id" ]] || { usage_terminal; return 2; }
    ...
    apply)
      ...
      [[ -n "$cur" ]] || { log_fail "no terminal configured"; return 1; }
