...
# Requires lib/paths.sh. theme_flavors, theme_is_flavor and theme_current need
# nothing else; the rendering functions also want lib/log.sh, lib/backup.sh
# (backup_link_target) and lib/iterm.sh (iterm_write_profiles).
...
theme_write_zsh() {
  ...
  title="$(printf '%s' "${flavor:0:1}" | tr '[:lower:]' '[:upper:]')${flavor:1}"
  while IFS= read -r h; do hexes+=("$h"); done < <(
    theme_hexes "$flavor" surface0 base rosewater red text mauve lavender surface1)
  [[ "${#hexes[@]}" -eq 8 ]] || { log_fail "palette lookup failed for $flavor"; return 1; }
